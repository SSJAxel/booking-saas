...
public record ProfessionalResponse(UUID id, UUID branchId, String displayName, String bio, boolean active) {
	...
	public static ProfessionalResponse from(Professional professional) {
		return new ProfessionalResponse(professional.getId(), professional.getBranchId(),
				professional.getDisplayName(), professional.getBio(), professional.isActive());
	}
}
