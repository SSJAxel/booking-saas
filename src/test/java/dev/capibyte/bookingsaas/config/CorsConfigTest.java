...
/** Local dev default is http://localhost:5180 (see application.yml app.cors.allowed-origins). */
class CorsConfigTest extends IntegrationTestBase {

	@Test
	void preflightFromAnAllowedOriginIsAccepted() {
		HttpHeaders headers = new HttpHeaders();
		headers.set(HttpHeaders.ORIGIN, "http://localhost:5180");
		...
		assertThat(response.getHeaders().getAccessControlAllowOrigin()).isEqualTo("http://localhost:5180");
	}
