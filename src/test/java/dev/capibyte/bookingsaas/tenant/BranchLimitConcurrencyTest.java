...
/**
 * Same guarantee as StockSaleConcurrencyTest, applied to the branch plan limit: proves the
 * pessimistic lock on the tenant row (TenantService#findByIdForUpdate) stops more simultaneous
 * creates than TRIAL's maxBranches=2 from all succeeding.
 */
class BranchLimitConcurrencyTest extends IntegrationTestBase {

	@Test
	void onlyAsManySimultaneousBranchesAsThePlanAllowsSucceed() throws Exception {
		RegisteredTenant tenant = registerTenant();
		HttpHeaders headers = authHeaders(tenant.token());
		// Tenants start on TRIAL by default (maxBranches=2) — no need to set the tier here.
		...
		for (int i = 0; i < attempts; i++) {
			...
			tasks.add(() -> {
				...
				ResponseEntity<Map> response = restTemplate.exchange("/api/branches", HttpMethod.POST,
						new HttpEntity<>(Map.of("name", "Branch " + n), headers), Map.class);
		...
		assertThat(succeeded).as("only TRIAL's maxBranches should win the race").isEqualTo(2);
